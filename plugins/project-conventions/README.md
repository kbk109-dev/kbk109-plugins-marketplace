...
| + 심볼 alternation | 11 | 6 |
| + 패턴 내 토큰 | **17** | 17 (30%) |
| **+ 대문자 토큰 제외 (현재)** | **17** | **13 (23%)** |

앞의 두 규칙은 코드 검색의 절반 이상을 통과시킨다. `export const appUser` 처럼 구조와 이름이 섞인
패턴, 그리고 모델이 심볼 여러 개를 한 번에 찾을 때 쓰는 alternation 을 놓치기 때문이다. **"코드 검색은
codegraph 로"가 목적인 이상 절반을 통과시키는 규칙은 목적을 달성하지 못한다.**

**대문자+밑줄 전용 토큰(`AUTOQA_ENV`·`DATABASE_URL`)은 신호로 치지 않는다.** 실측한 프로젝트에서
그런 토큰은 전부 환경변수·설정 키였고, 그건 심볼 조회가 아니라 설정 조회라 codegraph 가 답할 것이
없다. 대신 `export const MAX_RETRY` 같은 **대문자 코드 상수는 놓친다** — 측정에서 환경변수가 압도적으로
많아 내린 절충이고, `MAX_RETRY\|handleSubmit` 처럼 대소문자 혼합 토큰이 함께 있으면 그대로 잡힌다.

남은 오탐 13건은 `CREATE TABLE app_user`·`active_version_id` 처럼 **소문자 snake_case DB 식별자**가
섞인 검색이다. 받아들인 대가다 — 각 1회 재시도로 끝나고, 규칙 문서 2절에 경계를 명시했다.
근본 원인은 밑줄의 의미가 언어마다 다르다는 것이다(TypeScript 에서는 DB 관례, Python·Go 에서는 코드
관례). 패턴만으로는 구분되지 않으므로 언어별 판정은 넣지 않았다 — 틀렸을 때 원인을 찾기 어렵다.

단일 소문자 단어까지 넓히는 안도 함께 재봤는데 오탐만 8건(`auth`·`origin`·`outcome`) 늘고 새로 잡는
것이 없어 버렸다.
...
때문이다. 명령 치환 `$(grep …)` 은 **의도적으로 놓친다.**

**`xargs` 는 파이프 규칙의 예외다.** `find . -name '*.ts' | xargs grep -n appUser` 는 grep 이 파이프
뒤에 있지만 stdout 을 거르는 게 아니다 — xargs 가 **파일명을 인자로** 넘기므로 grep 은 트리를 훑는다.
파이프를 입은 소스 검색이고, 우회 의도 없이도 나오는 흔한 관용구라 예외로 잡는다. `xargs -0`·`-n 1`·
`-I {}` 처럼 값을 받는 플래그도 건너뛰고 실제 명령을 찾는다. `find . | xargs cat` 은 검색 바이너리가
아니므로 그대로 통과한다.
