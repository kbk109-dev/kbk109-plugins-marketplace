...
- `.claude/rules/notion-api-only.md` 가 이미 있는가 — 있으면 이미 설치된 것이니 새로 설치할지
  가 아니라 **재설치(갱신) 여부**를 묻는다
- `.claude/rules/` 아래에 이름에 `notion` 이 들어간 **다른** 규칙 파일이 있는가
  (`notion-api-only.md` 제외) — 있으면 이 프로젝트가 이미 이 플러그인과 무관한 자체 Notion
  거버넌스(예: 되돌리기 난이도별 쓰기 정책, 전용 스크립트)를 쓰고 있다는 뜻이다
...
**단, 마지막 신호(무관한 커스텀 Notion 규칙 존재)는 다른 세 신호와 다르게 다룬다.** 앞의
셋은 "Notion 을 쓰는가"의 근거일 뿐이지만, 이건 "이 프로젝트가 이미 다른 방식으로 관리
중"이라는 뜻이라 일반 템플릿을 그대로 설치하면 두 워크플로가 충돌한다. 이 경우 다른 근거와
나란히 나열하지 말고 **"이미 `{파일명}` 이 있습니다 — 일반 템플릿을 설치하면 그 규칙과
겹칠 수 있는데 그래도 설치할까요?"** 로 따로 명시해 되묻는다. 여기서도 자동으로 "아니오"
처리하지 않는다 — 두 체계를 의도적으로 병행하려는 경우일 수도 있으므로 판단은 사용자에게
맡긴다.
