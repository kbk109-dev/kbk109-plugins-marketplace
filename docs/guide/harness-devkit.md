...
> **범위** — 스킬 2개(`harness-dev`, `dev-monitor`). 훅·스크립트는 없다
...
### 알아 둘 한계 — 강제 수단은 문서뿐이다

이 플러그인에는 **스크립트도 훅도 없다.** 8가지 기계적 제약도, 하드 게이트 2개도, 실행 시점에
검사하는 코드가 아니라 SKILL.md 에 적힌 지시다. `project-conventions` 가 훅으로 도구 호출을
실제로 막는 것과 다르다.

그래서 규율이 깨지면 조용히 깨진다. **`feature_list.json` 의 `status` 와 `sprint_reports/` 의
채점 근거를 사람이 직접 들여다보는 것**이 유일한 실질적 검증 수단이다. 긴 작업일수록 스프린트가
끝날 때마다 한 번씩 열어 보는 편이 낫다.
