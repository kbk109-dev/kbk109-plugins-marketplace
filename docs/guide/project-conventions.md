...
flowchart TD
  ...
  T -. UserPromptSubmit 이 구조적으로 발화 못 함 .-> SA[서브에이전트]
  SA --> GREP[grep 부터 잡는다]
...
서브에이전트는 **사용자 프롬프트로 시작하지 않으므로** `UserPromptSubmit` 이 발화할 수 없고,
프로젝트 지시 상속도 보장되지 않는다. 그 결과 메인 세션은 codegraph 를 쓰는데 서브에이전트만
grep 부터 잡는다. 훅은 반드시 실행되는 지점 — 부모의 `Agent`/`Task` 도구 호출 — 을 잡아
프롬프트에 규칙을 물리적으로 심는다. **모델의 협조가 필요 없다.**
...
| 멱등 | 프롬프트에 이미 `codegraph` 가 있음 — 호출자의 탈출구이기도 하다 |

**전 구간 fail-open.** 예외·타임아웃·깨진 입력에도 항상 exit 0 한다. 서브에이전트 디스패치를
