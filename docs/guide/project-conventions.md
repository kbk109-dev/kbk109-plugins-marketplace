...
실행 전에 가장 궁금한 지점이다. **최대 6번**이고, 그 전까지 파일은 하나도 바뀌지 않는다.
...
| 5 | Step 1.6 | 컨텍스트 자동 압축 임계값을 낮출지 (기본 90%) | **항상 묻는다** — 탐지 근거가 없으므로 예/아니오만 받는다 |
| 6 | Step 2 | `--dry-run` 계획대로 설치할지 | **항상 묻는다** |
...
**`.claude/settings.local.json`(자동 압축 임계값)은 이 9가지에 없다 — 의도적이다.**
7·8·9 가 보는 대상은 **커밋되는** 정책 산출물이라 팀원 사이에 갈라지면 버그다. 반면
`settings.local.json` 은 gitignore 대상인 개인 파일이라, 팀원마다 값이 다르거나 아예
없는 것이 정상이다. 검사를 넣으면 그 설정을 켜지 않은 팀원 전원이 실패하게 된다.
