...
언급, `@notionhq/client`)에 두 신호를 추가한다: `.claude/rules/notion-api-only.md` 가 이미
설치돼 있는지, 그리고 그와 무관한 커스텀 Notion 규칙 파일이 이미 있는지("이미 **어떤
방식으로** 관리 중인가"). 후자를 발견하면 다른 근거처럼 조용히 나열하지 않고 — 일반
템플릿을 그대로 설치하면 두 워크플로가 충돌할 수 있으므로 — 별도로 명시해 되묻는다.

실사용 프로젝트가 이 스킬의 `notion-api-only` 템플릿과는 다른, 되돌리기 난이도별 쓰기
정책으로 진화한 자체 Notion 거버넌스를 이미 갖추고 있었는데, Step 1.5 가 그 존재를 감지하지
못해 재실행 시 일반 템플릿과 충돌할 뻔한 사례에서 나온 개선이다. 이번에도 자동으로
막거나 "아니오"로 단정하지 않는다 — 두 체계를 의도적으로 병행하려는 경우가 있을 수 있으므로
최종 판단은 그대로 사용자에게 남긴다.
...
**왜 `.claude/settings.json`(공유) 이 아니라 `.claude/settings.local.json`(개인) 인가.**
자동 압축 임계값은 모델·컨텍스트 크기·작업 습관에 딸린 개인 환경 값이지 프로젝트 규약이
아니다. 커밋되는 파일에 쓰면 이 저장소를 클론한 모든 팀원의 개인 `~/.claude/settings.json`
설정을 덮어쓴다 — AGENTS.md 가 `.mcp.json` 재정의를 기각한 것과 같은 논리다.
`settings.local.json` 은 gitignore 대상이므로 이 충돌이 구조적으로 발생하지 않는다.
설치 스크립트는 그 파일이 실제로 `.gitignore` 대상인지 확인해, 아니면 경고만 출력한다
(`.gitignore` 자체는 고치지 않는다).

**"커밋 후 자동 압축"은 별도로 검토해 기각했다.** Claude Code 훅에는 압축을 발동시키는
출력 필드가 없고, `PreCompact` 훅은 이미 시작된 압축을 **차단**만 할 수 있으며, 모델은
`/compact` 같은 내장 슬래시 명령을 호출할 도구를 갖고 있지 않다. 자동 압축이 반응하는
유일한 신호는 이번에 추가한 **토큰 임계값**뿐이다 — "작업 완료 후"라는 트리거는
하네스가 지원하지 않는다.

`check-agent-rules` 는 이 파일을 검사하지 않는다 — 팀원마다 값이 다르거나 아예 없는 것이
정상이라, 커밋되는 정책 산출물(검사 7·8·9)과는 성격이 다르다.
...
검사하는 코드로 옮기고, 코드가 볼 수 없는 셋은 컨텍스트 압축을 견디는 자리로 옮긴다.

### ⚠️ 업데이트하면 달라지는 것

**이 플러그인에 훅이 생긴다.** `PreToolUse` 로 `Write|Edit` 를 받아 `docs/harness/*/feature_list.json`
쓰기만 판정한다. 켜진 모든 프로젝트에서 발화하지만 **첫 분기가 `file_path` 문자열 검사뿐**이라
무관한 호출은 파일을 열기도 전에 빠진다. 대신 호출당 파이썬 기동 비용(수십 ms)이 상시로 붙는다 —
`project-conventions` 가 Bash 매처에 대해 감수한 것과 같은 종류의 비용이다.
...
- **`validate_feature_list.py`** — 제약 2(criteria 불변) · 6(JSON 유지) · 7(재시도 상한) ·
  ...
  위반으로 잡힌다. 순서만 바꾼 것은 위반이 아니고(내용이 같다), 문구를 고치거나 항목을 지우면
  잡힌다.

- **`feature_list.json` 규율 훅** — 쓰기를 가로채 결과 내용을 미리 판정하고 위반이면 `deny` 한다.
  훅과 스크립트가 `hooks/_feature_list_rules.py` 하나를 공유한다. 규칙을 복제하면 훅의 판정과
  사람이 돌리는 검사가 갈라지는데, 그게 이 플러그인이 막으려는 실패와 같은 종류다.

  **이 훅은 제약을 불가능하게 만들지 못한다.** 플러그인 훅이 전역 발화해도 안전하려면 차단이
  복구 가능해야 하므로, 같은 `(에이전트, 위반)` 은 한 번만 차단하고 재호출은 통과시킨다. 훅이
  만드는 것은 **조용한 지름길을 의도적인 선택으로 바꾸는 것**이다. 불가능하게 만드는 쪽은
  스프린트마다 도는 검증 스크립트이고, 사람이 그 결과를 본다.

  키를 파일 내용이 아니라 **위반의 종류**로 잡는다. 내용 해시로 잡으면 무관한 한 글자만 바꿔도
  새 키가 되어 매번 차단되고, 그게 곧 루프다.

- **`harness_agents_block.py`** — 작업 규율을 대상 프로젝트 `AGENTS.md` 에 마커 블록으로 설치한다.
  `project-conventions` 와 이름공간이 달라(`harness-dev` vs `agent-rules: {name}`) 한 파일에
  공존해도 서로를 건드리지 않는다.

  **여기에는 진행 상태를 쓰지 않는다.** slug 도 "진행 중" 도 없다 — 그건 `docs/harness/*/` 를
  보면 아는 사실이고, 두 번째 사본을 만들면 중단된 작업에서 AGENTS.md 가 **틀린 지시**를 하게
  된다. 진행 상태는 `progress.md` 와 `feature_list.json` 의 몫이고 이 블록은 실행과 무관한 항구적
  규약만 담는다. 그래서 slug 인자가 없고, 재설치가 멱등이며, 작업이 끝나도 제거하지 않는다.
  담당하는 것은 코드가 검사할 수 없는 제약 1·3·5 뿐이다.

- **Phase 0.5 — 상태 복원 게이트.** `docs/harness/` 에 폴더가 있으면 새로 시작하는 게 아니다.
  `progress.md` → `feature_list.json` 을 읽기 전에는 구현하지 않는다. `dev-monitor` 가 이미
  쓰던 형태인데 `harness-dev` 에는 빠져 있었다 — 상태를 외부화해 놓고 읽기를 강제하지 않으면
  외부화가 무의미하다.
